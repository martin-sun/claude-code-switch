...
ccc() {
  if [[ \$# -eq 0 ]]; then
    ...
    echo "  Official: deepseek, glm, kimi, kimicode(kc), qwen, claude, opus, haiku, longcat, minimax, zenmux, kat"
  ...
  _is_known_model() {
    case "\$1" in
      deepseek|ds|glm|glm4|glm4.6|kimi|kimi2|kimicode|kc|qwen|longcat|lc|minimax|mm|zenmux|kat|claude|sonnet|s|opus|o|haiku|h)
        return 0 ;;
      *)
        return 1 ;;
    esac
